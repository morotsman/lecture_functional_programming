describe('lodash', function () {
    ...
    it("add 1 to each number in the list", function(){
        ...
        const actual = _(numbers).map(number => number + 1).value();
    ...
    it("get length of each word in the list", function(){
        ...
        const actual = _(words).map(word => word.length).value();
    ...
    it("keep all the triangles", function(){
		...
		const actual2 = _(shapes).filter(isTriangle).value();
    ...
    it("keep all even numbers", function(){
		...
		const actual = _(numbers).filter(isEven).value();
    ...
    it("find the first name of all parents", function(){
        const parents = _(persons).filter(isParent).map(fullName).value();
    ...
    it("group all persons together on last name", function(){
        const actual = _(persons)
			.groupBy('lastName')
			.map(group => group.map(fullName)).value();
    ...
    it("group all persons together on last name, only return the youngest person", function(){
        const actual = _(persons)
			.groupBy('lastName')
			.map(group => _(group).orderBy('age').map(fullName).first()).value();
    ...
    it("partition the persons in children and parents", function(){
        const actual = _(persons)
			...
			.partition(isParent)
			.map(partition => partition.map(fullName)).value();
    ...
    it("find youngest child and parent", function(){
        const actual = _(persons)
			...
			.partition(isParent)
			.map(partition => _(partition).orderBy('age').map(fullName).first()).value();
    ...
    it("find ", function(){
        const actual = permutations(persons)
			...
			}).filter(persons => persons.difference > 6 && persons.difference < 10).value();
